/** 
 * TREE_CHAIN_SPLIT_P3384_2_CPP
 * @author : MaWeiTao 
 * @date : 20:41 2020/3/14 
 *
 */
#include <iostream>
#include <cstdio>
...
namespace TREE_CHAIN{
    inline int query_path(int x,int y){
        ...
        while(top[x]!=top[y]){//???????????????
            ...
            res=0;
            query(1,1,n,id[top[x]],id[x]);//ans?????x????????????????????
            ans+=res;
            ans%=mod;//??????
            x=fa[top[x]];//?????x????????????????????
        }
    ...
    inline void dfs1(int x,int f,int deep){//x??????f?????deep???
        dep[x]=deep;//??????????
        fa[x]=f;//??????????
        siz[x]=1;//?????????????????
        int maxson=-1;//????????????
    ...
    inline void dfs2(int x,int tp){//x??????topf?????????????
        id[x]=++cnt;//??????????
        wt[cnt]=w[x];//?????????????????????
        top[x]=tp;//??????????????
        if(!son[x])return;//?????????????
